        ...
        >
          <div className="text-left space-y-10">
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center gap-6 pt-4">
              <button 
                onClick={onEnter}
                className="w-full sm:w-auto px-10 py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-2xl shadow-blue-500/20 active:scale-95 transition-all"
              >
                Launch Terminal
                <ArrowRight size={18} />
              </button>
              <div className="flex flex-col">
                <div className="flex -space-x-3 overflow-hidden p-1">
                  {['AZ', 'FB', 'LL', 'GK'].map(initials => (
                    <div key={initials} className="inline-block h-8 w-8 rounded-full ring-4 ring-[#07090e] bg-zinc-800 border border-zinc-700 flex items-center justify-center text-[8px] font-black uppercase text-zinc-400">
                      {initials}
                    </div>
                  ))}
                </div>
                <div className="mt-1 text-[10px] text-zinc-500 font-black uppercase tracking-widest">
                  Trusted by Global Freight Forwarders
                </div>
              </div>
            </motion.div>
          </div>
            ...
            <div className="relative z-10 bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-6 shadow-2xl glow-blue overflow-hidden group">
              ...
              <div className="space-y-6 font-mono">
                ...
                <div className="p-6 bg-red-500/5 border border-red-500/20 rounded-2xl space-y-4 relative">
                  ...
                  <button onClick={onEnter} className="w-full py-3 bg-red-500 text-white font-black uppercase text-[9px] tracking-widest rounded-xl hover:bg-red-600 transition-all">
                    Draft Auto-Dispute
                  </button>
                </div>
              </div>
          ...
          <div className="p-16 rounded-[3.5rem] bg-[#0c0f17] border-2 border-blue-500/30 shadow-2xl relative overflow-hidden">
            ...
            <ul className="space-y-6 mb-12">
              {PRICING_PLAN.features.map((f, i) => (
                <li key={i} className="flex items-start gap-4 text-sm text-zinc-300 font-bold">
      ...
      <footer className="py-24 px-6 border-t border-zinc-900/50 bg-[#07090e]">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-16">
          <div className="space-y-8">
            ...
            <p className="text-xs text-zinc-600 font-bold leading-relaxed uppercase tracking-widest">Defending global logistics margins with institutional-grade AI.</p>
          </div>
          {[
            { title: "Platform", links: ["Intelligence Feed", "Lane Memory", "Review Queue", "Scorecards"] },
            ...
            <div key={i} className="space-y-8">
              ...
              <ul className="space-y-4">
                {col.links.map((l, j) => (
                  <li key={j}>
                    <button onClick={() => setActiveOverlay(l.toLowerCase() as any)} className="text-[11px] font-black uppercase tracking-widest text-zinc-600 hover:text-white transition-colors">{l}</button>
                  </li>
                ))}
              </ul>
            </div>
